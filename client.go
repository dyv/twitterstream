...
func (c *Client) Sample() (*Connection, error) {
	form := url.Values{"language": {"en"},
		"filter_level": {"medium"}}
	return c.sample(form)
}
...
func (c *Client) sample(form url.Values) (*Connection, error) {
	req, err := http.NewRequest("POST", SampleUrl, strings.NewReader(form.Encode()))
	if err != nil {
